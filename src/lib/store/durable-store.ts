...
import {
  ...
  setPersistentRecordIfAbsentWithTtl,
  withPersistentUsage,
...
export async function setDurableRecordIfAbsentWithTtl(
  collection: PersistentCollection,
  id: string,
  value: unknown,
  ttlSeconds: number
): Promise<boolean> {
  return setPersistentRecordIfAbsentWithTtl(collection, id, value, ttlSeconds);
}
