 ...
 * @okxweb3/a2a-openclaw@0.1.10 plugin's own internal Gateway token
 ...
 * (traced to `function Fe(){ let n=D().config.loadConfig().gateway; return
 * {..., token: Ue(n?.auth?.token), ...} }`), through its own resolver:
 *
 *   function Ue(e){if(e){if(typeof e=="string")return e;if(typeof e=="object"
 *   &&e!==null&&"env"in e)return process.env[e.env]||void 0}}
 *
 * `Ue` only understands a plain string or the plugin-native `{env:
 ...
 * id:"OPENCLAW_GATEWAY_TOKEN"}` — an object with no `env` key — so `Ue`
 ...
 * end to end (72,924 bytes): no reference to any plugin-scoped config key,
 * `OPENCLAW_GATEWAY_TOKEN`, or `OKX_A2A_OPENCLAW_GATEWAY_TOKEN` exists in
 * it at all.
 *
 * === The patch ===
 * A single, narrow, additive branch: when `Ue` receives an object with
...
/** Pinned against @okxweb3/a2a-openclaw@0.1.10's real published dist/index.js — computed directly from the exact tarball Dockerfile.seller extracts (same npm pack + integrity-checked source), not guessed. */
...
export const ORIGINAL_TOKEN_RESOLVER_SOURCE =
  'function Ue(e){if(e){if(typeof e=="string")return e;if(typeof e=="object"&&e!==null&&"env"in e)return process.env[e.env]||void 0}}';
...
export const PATCHED_TOKEN_RESOLVER_SOURCE =
  'function Ue(e){if(e){if(typeof e=="string")return e;if(typeof e=="object"&&e!==null&&"env"in e)return process.env[e.env]||void 0;if(typeof e=="object"&&e!==null&&e.source==="env"&&typeof e.id=="string")return process.env[e.id]||void 0}}';
  ...
  if (actualSha256 !== EXPECTED_UPSTREAM_SHA256) {
    throw new TokenResolverPatchError(
      `refusing to patch ${pluginDistIndexPath}: sha256 is ${actualSha256}, expected ${EXPECTED_UPSTREAM_SHA256} ` +
        `(pinned from @okxweb3/a2a-openclaw@0.1.10's real published dist/index.js). The upstream source no longer ` +
