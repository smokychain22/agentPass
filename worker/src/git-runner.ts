...
import type { RepositoryJob } from "../../src/lib/worker/types";
...
export async function cloneExactCommit(input: {
  ...
  if ((status.stdout ?? "").trim()) {
    throw new Error("Workspace is not clean after checkout.");
  }
...
export async function generateGitPatch(
  ...
  const { applyEditsToWorkspace } = await import("../../src/lib/patch-kit/canonical-patch");
...
export function workspaceLayout(cleanupRunId: string): string {
  const safe = cleanupRunId.replace(/[^a-zA-Z0-9_-]/g, "_");
  return path.join(os.tmpdir(), "repodiet", safe);
}
